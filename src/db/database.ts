...
async function applyMigrationV3(db: SQLiteDatabase): Promise<void> {
  await db.execAsync(`
    ...
    INSERT INTO categories_v3 (id, user_id, name, category_type, created_at)
    SELECT id, user_id, name, category_type, created_at FROM categories;
    ...
    INSERT INTO transactions_v3 (id, user_id, account_id, category_id, amount, transaction_type, description, date, created_at)
    SELECT id, user_id, account_id, category_id, amount, transaction_type, description, date, created_at FROM transactions;
  ...
  const timestamp = new Date().toISOString();
  const debtPaymentCategories = [
    "Pago de deuda",
    "Abono a préstamo",
  ];

  for (const name of debtPaymentCategories) {
    await db.runAsync(
      `
      INSERT OR IGNORE INTO categories (user_id, name, category_type, created_at)
      SELECT id, ?, 'DEBT_PAYMENT', ? FROM users;
      `,
      [name, timestamp],
    );
  }
